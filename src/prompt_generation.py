...
def prepare_metadata_ham10000(row: pd.Series) -> str:
    """Prepare metadata string for the HAM10000 dataset"""
...
def generate_prompts_dataset(
    ...
    if dataset == "ham10000":
        prepare_metadata = prepare_metadata_ham10000
    elif dataset == "derm12345":
    ...
    for idx, row in metadata.iterrows():
        ...
        if dataset == "ham10000":
            ...
            # Map the label
            label = ham10000_label_map[label]

            # Replace placeholders in prompts with actual metadata
            general_prompt_filled = general_prompt.format(label=label)
            ...
            image_path = os.path.join(dataset_path, row["image_id"] + ".jpg")
